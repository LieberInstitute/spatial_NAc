...
#Convert log.FDR to FDR
markers_1vALL_sub$FDR <- exp(markers_1vALL_sub$log.FDR)

#Make a column that depicts type of testing. 
markers_1vALL_sub$Testing_Type <- "1vALL"

#Reorder the columns. 
markers_1vALL_sub <- markers_1vALL_sub[,c("gene_id","gene_name","FDR","std.logFC","Testing_Type","cellType.target")]

#Convert std.logFC to logFC. Make sure to note in the caption that logFC for 1vALL testing is the standardized logFC. 
colnames(markers_1vALL_sub)[4] <- "logFC"

markers_1vALL_sub <- subset(markers_1vALL_sub,subset=(FDR <= 0.05))
...
colnames(markers_pairwise_df)[4] <- "logFC"
markers_pairwise_df$Testing_Type <- "Pairwise"

#reorder the dataframe
markers_pairwise_df <- markers_pairwise_df[,c("gene_id","gene_name","FDR","logFC","Testing_Type","cellType.target")]

#remove the rownaems
rownames(markers_pairwise_df) <- NULL

head(markers_pairwise_df)
head(markers_1vALL_sub)

#Combine them for one table. 
DEG_table <- rbind(markers_1vALL_sub,markers_pairwise_df)

#Write the supplementary table
write_xlsx(
  list(Sheet1 = DEG_table),
