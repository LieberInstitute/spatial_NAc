...
library(dplyr)
# read data 
dat_dir <- here("processed-data", "19_sLDSC_old", "snRNA_seq")
...
sig.ldsc$trait.ordered = factor(sig.ldsc$trait, levels=c("Education Years","Intelligence","Neuroticism",  "Bipolar","Schizophrenia","Depression",
                                                         "Smoking initiation", "Cigarettes per day","Drinks per week", "Alzheimer Disease", "BMI"))

palette1= RColorBrewer::brewer.pal(n=7,"RdYlBu")[7:1]
plotDir <- here("plots", "19_sLDSC_old", "snRNA_seq")

sig.ldsc$cell.ordered = gsub("_", " ", sig.ldsc$cell)
sig.ldsc$cell.ordered = factor(sig.ldsc$cell.ordered, levels=c("DRD1 MSN B","DRD1 MSN C", "DRD1 MSN D","DRD2_MSN_B", "Excitatory","Inh B", "Endothelial", "OPC", "Microglia"))
