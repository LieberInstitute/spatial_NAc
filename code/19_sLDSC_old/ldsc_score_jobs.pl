...
open(OUT, ">ldsc_score_jobs_snRNA_seq.txt");
...
foreach $out (@outdir){
  foreach $i(1..22){
	...
	$anno="/dcs04/lieber/marmaypag/spatialNac_LIBD4125/spatial_NAc/processed-data/19_sLDSC_old/snRNA_seq/input_files/$out/chr.".$i.".annot.gz";
	$hapmapSNP=$hapmap3."hm.".$i.".snp";
	print OUT "python $ldsc --l2 --thin-annot --ld-wind-cm 1 ";
	print OUT "--bfile $bfile ";
	print OUT "--anno $anno ";
	print OUT "--out /dcs04/lieber/marmaypag/spatialNac_LIBD4125/spatial_NAc/processed-data/19_sLDSC_old/snRNA_seq/input_files/$out/chr.$i ";
