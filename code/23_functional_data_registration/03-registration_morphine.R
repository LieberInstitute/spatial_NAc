...
morphine_acute <- read.csv(file.path(res_dir, "raw-data", "morphine_acute.csv"))
morphine_chronic <- read.csv(file.path(res_dir, "raw-data", "morphine_chronic.csv"))
colnames(morphine_acute) <- c("Cluster","Cell_type", "Gene", "Ensembl.ID", "logFC", "q.value")
colnames(morphine_chronic) <- c("Cluster","Cell_type", "Gene", "Ensembl.ID", "logFC", "q.value")
...
upregulated_acute <- subset(morphine_acute, logFC > 0)
downregulated_acute <- subset(morphine_acute, logFC < 0)
upregulated_chronic <- subset(morphine_chronic, logFC > 0)
downregulated_chronic <- subset(morphine_chronic, logFC < 0)
...
morphine_enrichment_upregulated_acute <- gene_set_enrichment(
    gene_list = upregulated_genes_acute_ensembl,
    fdr_cut = 0.05,
    modeling_results = modeling_results,
    model_type = "enrichment"
)
morphine_enrichment_downregulated_acute <- gene_set_enrichment(
    gene_list = downregulated_genes_acute_ensembl,
    fdr_cut = 0.05,
    modeling_results = modeling_results,
    model_type = "enrichment"
)
morphine_enrichment_upregulated_chronic <- gene_set_enrichment(
    gene_list = upregulated_genes_chronic_ensembl,
    fdr_cut = 0.05,
    modeling_results = modeling_results,
    model_type = "enrichment"
)
morphine_enrichment_downregulated_chronic <- gene_set_enrichment(
    gene_list = downregulated_genes_chronic_ensembl,
    fdr_cut = 0.05,
    modeling_results = modeling_results,
    model_type = "enrichment"
)
...
plot_results <- function(gene_lists_human, modeling_results, morphine_enrichment, fileName){
    gene_count_col <- get_gene_list_count(gene_lists_human)
    gene_count_row <- get_gene_enrichment_count(model_results = modeling_results, fdr_cut = 0.05)
    morphine_enrichment$test <- gsub("\\.", " ", morphine_enrichment$test)
    morphine_enrichment$test[morphine_enrichment$test == "Endothelial Ependymal"] <- "Endothelial/Ependymal"

    rownames(gene_count_row) <- gsub("\\.", " ", rownames(gene_count_row))
    rownames(gene_count_row)[rownames(gene_count_row) == "Endothelial Ependymal"] <- "Endothelial/Ependymal"
    plot_dir <- here::here("plots", "23_functional_data_registration", "morphine")
    pdf(file.path(plot_dir, fileName), width = 8.5, height = 4)
    gene_set_enrichment_plot_complex(
        enrichment = morphine_enrichment,
...
plot_results(upregulated_genes_acute_ensembl, modeling_results, morphine_enrichment_upregulated_acute, "Morphine_upregulated_acute.pdf")
plot_results(upregulated_genes_chronic_ensembl, modeling_results, morphine_enrichment_upregulated_chronic, "Morphine_upregulated_chronic.pdf")
plot_results(downregulated_genes_acute_ensembl, modeling_results, morphine_enrichment_downregulated_acute, "Morphine_downregulated_acute.pdf")
plot_results(downregulated_genes_chronic_ensembl, modeling_results, morphine_enrichment_downregulated_chronic, "Morphine_downregulated_chronic.pdf")

save_dir <- here::here("processed-data", "23_functional_data_registration", "morphine")
saveRDS(morphine_enrichment_upregulated_acute, file.path(save_dir, "morphine_enrichment_upregulated_acute.rds"))
saveRDS(morphine_enrichment_downregulated_acute, file.path(save_dir, "morphine_enrichment_downregulated_acute.rds"))
saveRDS(morphine_enrichment_upregulated_chronic, file.path(save_dir, "morphine_enrichment_upregulated_chronic.rds"))
saveRDS(morphine_enrichment_downregulated_chronic, file.path(save_dir, "morphine_enrichment_downregulated_chronic.rds"))
