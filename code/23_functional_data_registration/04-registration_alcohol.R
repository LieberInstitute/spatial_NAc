...
# Read in the AUD genes
DEG_dir <- here("processed-data", "23_functional_data_registration", "raw-data")
DEG_file <- file.path(DEG_dir, "NAc_LIBD_alcohol.csv")
DEG_results <- read.csv(DEG_file)
DEG_results <- DEG_results[!is.na(DEG_results$adjusted_pvalue), ]
DEG_results$gene_id <- gsub("\\..*$", "", DEG_results$gencode_id)

upregulated_DEGs <- DEG_results[DEG_results$log2_fold_change > 0, ]
downregulated_DEGs <- DEG_results[DEG_results$log2_fold_change < 0, ]


upregulated_gene_lists <- list()
upregulated_gene_lists[["FDR < 0.05"]] <- upregulated_DEGs$gene_id[upregulated_DEGs$adjusted_pvalue < 0.05]
upregulated_gene_lists[["FDR < 0.1"]] <- upregulated_DEGs$gene_id[upregulated_DEGs$adjusted_pvalue < 0.1]
upregulated_gene_lists[["FDR < 0.2"]] <- upregulated_DEGs$gene_id[upregulated_DEGs$adjusted_pvalue < 0.2]
upregulated_gene_lists[["FDR < 0.3"]] <- upregulated_DEGs$gene_id[upregulated_DEGs$adjusted_pvalue < 0.3]

downregulated_gene_lists <- list()
downregulated_gene_lists [["FDR < 0.1"]] <- downregulated_DEGs$gene_id[downregulated_DEGs$adjusted_pvalue < 0.1]
downregulated_gene_lists [["FDR < 0.2"]] <- downregulated_DEGs$gene_id[downregulated_DEGs$adjusted_pvalue < 0.2]
downregulated_gene_lists [["FDR < 0.3"]] <- downregulated_DEGs$gene_id[downregulated_DEGs$adjusted_pvalue < 0.3]


alcohol_enrichment_upregulated <- gene_set_enrichment(
    gene_list = upregulated_gene_lists,
    fdr_cut = 0.05,
    modeling_results = modeling_results,
    model_type = "enrichment"
)

alcohol_enrichment_downregulated <- gene_set_enrichment(
    gene_list = downregulated_gene_lists,
    fdr_cut = 0.05,
    modeling_results = modeling_results,
    model_type = "enrichment"
)

saveRDS(alcohol_enrichment_upregulated, file.path(res_dir, "alcohol_enrichment_upregulated.rds"))
saveRDS(alcohol_enrichment_downregulated, file.path(res_dir, "alcohol_enrichment_downregulated.rds"))


source(here("code", "23_functional_data_registration", "gene_set_enrichment_plot_complex.R"))

gene_count_col <- get_gene_list_count(upregulated_gene_lists)
gene_count_row <- get_gene_enrichment_count(model_results = modeling_results, fdr_cut = 0.05)
alcohol_enrichment_upregulated$test <- gsub("\\.", " ", alcohol_enrichment_upregulated$test)
alcohol_enrichment_upregulated$test[alcohol_enrichment_upregulated$test == "Endothelial Ependymal"] <- "Endothelial/\nEpendymal"
...
pdf(file.path(plot_dir, "final_enrichment_plot_upregulated.pdf"), width = 8, height = 5)
gene_set_enrichment_plot_complex(
    enrichment = alcohol_enrichment_upregulated,
...
gene_count_col <- get_gene_list_count(downregulated_gene_lists)
gene_count_row <- get_gene_enrichment_count(model_results = modeling_results, fdr_cut = 0.05)
alcohol_enrichment_downregulated$test <- gsub("\\.", " ", alcohol_enrichment_downregulated$test)
alcohol_enrichment_downregulated$test[alcohol_enrichment_downregulated$test == "Endothelial Ependymal"] <- "Endothelial/\nEpendymal"

rownames(gene_count_row) <- gsub("\\.", " ", rownames(gene_count_row))
rownames(gene_count_row)[rownames(gene_count_row) == "Endothelial Ependymal"] <- "Endothelial/\nEpendymal"

pdf(file.path(plot_dir, "final_enrichment_plot_downregulated.pdf"), width = 8, height = 5)
gene_set_enrichment_plot_complex(
    enrichment = alcohol_enrichment_downregulated,
    gene_count_col = gene_count_col,
    gene_count_row = gene_count_row,
    PThresh = 12,
    ORcut = 1,
