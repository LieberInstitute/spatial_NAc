...
a = readImgData(
    path = file.path(transformed_dir, unique(sample_info$Brain)),
    sample_id = unique(sample_info$Brain),
    imageSources = file.path(
        transformed_dir, unique(sample_info$Brain), "tissue_hires_image.png"
    ),
    scaleFactors = file.path(
        transformed_dir, unique(sample_info$Brain), "scalefactors_json.json"
    ),
    load = TRUE
)
