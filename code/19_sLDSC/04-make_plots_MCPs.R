...
# read data 
dat_dir <- here("processed-data", "19_sLDSC", "meringue_factors")
...
sig.ldsc$trait.ordered = factor(sig.ldsc$trait,levels=c("Education Years", "Intelligence", "Neuroticism", "Bipolar", 
                                                        "Schizophrenia", "Depression", "Smoking cessation", "Smoking initiation",
                                                        "Drinks per week",  "Epilepsy", "Height"))
palette1= RColorBrewer::brewer.pal(n=7,"RdYlBu")[7:1]
plotDir <- here("plots", "19_sLDSC", "meringue_factors")
sig.ldsc$cell.ordered = sig.ldsc$cell
sig.ldsc$cell.ordered[sig.ldsc$cell.ordered == "meringue_cluster_1"] <- "MCP_1"
sig.ldsc$cell.ordered[sig.ldsc$cell.ordered == "meringue_cluster_2"] <- "MCP_2"
sig.ldsc$cell.ordered[sig.ldsc$cell.ordered == "meringue_cluster_3"] <- "MCP_3"
sig.ldsc$cell.ordered[sig.ldsc$cell.ordered == "meringue_cluster_4"] <- "MCP_4"
sig.ldsc$cell.ordered <- factor(sig.ldsc$cell.ordered, levels = c("MCP_1", "MCP_2", "MCP_3", "MCP_4"))

pdf(file.path(plotDir, "significant_traits.pdf"), width = 5, height = 4)
ggplot(sig.ldsc, aes(x=cell.ordered, y=trait.ordered, size=-log10(FDR), color=Coefficient_z.score))+
  ...
  scale_color_gradientn(colors=c(palette1[1:2],"lightgrey",palette1[6:7]),
                        values=c(0,.2,.4,.8,1))+
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=.5)) + xlab("Meringue consensus pattern") + ylab("Trait") 
dev.off()
