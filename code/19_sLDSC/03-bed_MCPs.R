...
# Load libraries
library(here)
cell <- read.csv(here::here("processed-data", "19_sLDSC", "meringue_factors", "input_files", "meringue_score.csv"))
colnames(cell)[1] <- "geneName"
colnames(cell) <- gsub("\\.", "_", colnames(cell))
...
for(i in 1:length(modules)){
    ...
    outfile <- here::here("processed-data", "19_sLDSC", "meringue_factors","input_files", "bedfiles", filename)
