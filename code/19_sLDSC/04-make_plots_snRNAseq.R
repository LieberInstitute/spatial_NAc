...
library(dplyr)
# read data 
dat_dir <- here("processed-data", "19_sLDSC", "snRNA_seq")
...
sig.ldsc$trait.ordered = factor(sig.ldsc$trait, levels=c("Education Years","Intelligence","Neuroticism",  "Bipolar","Schizophrenia","Depression",
                                                        "Smoking initiation","Smoking cessation", "Cigarettes per day", "Drinks per week",
                                                         "Alzheimer Disease","BMI", "Height"))

palette1= RColorBrewer::brewer.pal(n=7,"RdYlBu")[7:1]
plotDir <- here("plots", "19_sLDSC", "snRNA_seq")

sig.ldsc$cell.ordered = gsub("_", " ", sig.ldsc$cell)
#sig.ldsc$cell.ordered = factor(sig.ldsc$cell.ordered, levels=c("DRD2 MSN A","DRD1 MSN C", "DRD2 MSN B", 
# "DRD1 MSN B", "DRD1 MSN D", "Excitatory", "Inh B", "Microglia", "Astrocyte B", "Ependymal", "Endothelial", "Neuron Ambig"))

sig.ldsc$cell.ordered = factor(sig.ldsc$cell.ordered, levels = c("DRD1 MSN A", "DRD2 MSN A", "DRD2 MSN B","DRD1 MSN C", "DRD1 MSN B","DRD1 MSN D",
  "Excitatory", "Inh B","Inh F",  "Astrocyte B", "Microglia","Ependymal", "Endothelial"))
...
ggplot(sig.ldsc, aes(x=cell.ordered, y=trait.ordered, size=-log10(FDR), color=Coefficient_z.score))+
  ...
  scale_color_gradientn(colors=c(palette1[1:2],"lightgrey",palette1[6:7]),
                        values=c(0,.2,.4,.8,1))+
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=.5)) + xlab("Cell type") + ylab("Trait") 
dev.off()
