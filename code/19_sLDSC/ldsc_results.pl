...
opendir($dh, "/dcs04/lieber/marmaypag/spatialNac_LIBD4125/spatial_NAc/processed-data/19_sLDSC/meringue_factors/sLDSC_coefficients/");
@outdirs = grep { !/^\./} readdir($dh);
closedir $dh;

$count=0;

open(OUT, ">/dcs04/lieber/marmaypag/spatialNac_LIBD4125/spatial_NAc/processed-data/19_sLDSC/meringue_factors/ldsc_results.txt");
...
foreach $dir (@outdirs){
	...
	print "$count\n";
	$group=$dir;
	chdir "/dcs04/lieber/marmaypag/spatialNac_LIBD4125/spatial_NAc/processed-data/19_sLDSC/meringue_factors/sLDSC_coefficients/${dir}";
