...
# ----------------------------
gseaPlotMCP <- function(mcp_name, mcp_short, mcp_terms) {

  results <- read.csv(
    file.path(save_dir, paste0(mcp_short, "_reactome_results.csv")),
    row.names = 1
  )
  ...
  # safely subset positively correlated genes
  idx <- !is.na(cor_mat[, mcp_name]) & cor_mat[, mcp_name] > 0
  non0.mcp <- rownames(cor_mat)[idx]
  non0.mcp <- non0.mcp[!is.na(non0.mcp)]
  ...
  keep <- !is.na(names(non0.id)) &
    !is.na(non0.id) &
    names(non0.id) %in% rownames(cor_mat)

  non0.id <- non0.id[keep]

  mcp.stats <- cor_mat[names(non0.id), mcp_name]
  names(mcp.stats) <- non0.id
  mcp.stats <- sort(mcp.stats, decreasing = TRUE)

  mcp.df <- do.call(rbind, lapply(seq_len(nrow(tmp)), function(i) {
    term1 <- tmp$pathway[i]
  ...
  ggplot(
    dplyr::filter(mcp.df, present == TRUE),
    aes(x = rank, xend = rank, y = y, yend = yend, color = term)
  ) +
    geom_segment(linewidth = 0.6) +
    scale_x_continuous(
      limits = c(0, max(mcp.df$rank)),
      expand = c(0, 0)
    ) +
    scale_y_continuous(
...
# ----------------------------
combined_plot <- gridExtra::grid.arrange(p1, p3, p4, ncol = 1)
