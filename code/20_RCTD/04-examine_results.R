...
RCTD_dir <- here::here("processed-data", "20_RCTD", sample_id)
if(opt$marker_genes){
...
plot_dir <- here("plots", "20_RCTD", sample_id)
pdf(file.path(plot_dir,"multi_allWeight_markers.pdf"), width = 5, height = 5)
...
pdf(file.path(plot_dir,"multi_subWeight_markers.pdf"), width = 5, height = 5)
for(i in c(1:length(plot_list))){
    print(plot_list[[i]])
}
dev.off()